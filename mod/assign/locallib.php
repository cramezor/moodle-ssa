        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    ...
                    // RT - Downloaded filename to contain student ID and name
                    //$prefix = str_replace('_', ' ', $groupname . fullname($student));
                    //$prefix = clean_filename($prefix . '_' . $this->get_uniqueid_for_user($userid) . '_');
					$prefix = clean_filename($student->username . "_" . fullname($student) . "_" .$student->id . "_");
                }
                        ...
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
                            ...
                            foreach ($pluginfiles as $zipfilename => $file) {
                                ...
                                $prefixedfilename = clean_filename($prefix .
                                                                   //$subtype . // RT - don't include part of filename
                                                                   //'_' .
                                                                   $type .
                                                                   '_' .
                                                                   $zipfilename);
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                ...
                if (!$gradingdisabled) {
                    ...
                    $mform->setType('grade', PARAM_FLOAT); // RT - value should be numeric
					$mform->addRule('grade', null, 'numeric', null, 'client'); // RT - Numeric validation
                } else {
